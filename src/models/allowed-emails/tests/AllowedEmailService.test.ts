...
describe('AllowedEmailService', () => {
  ...
  describe('bulkAdd', () => {
    ...
    it('correctly separates added from skipped (already-existing) emails', async () => {
      ...
      const result = await service.bulkAdd(SCHOOL_ID, ['new@school.com', 'existing@school.com'], USER_ID);
    ...
    it('throws on database error', async () => {
      ...
      await expect(service.bulkAdd(SCHOOL_ID, ['a@school.com'], USER_ID)).rejects.toThrow('insert failed');
    });
  });
