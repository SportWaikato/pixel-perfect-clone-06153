...
describe('isEmailAllowedResult', () => {
  ...
  it('undefined → blocked (RPC error path — must fail closed)', () => expect(isEmailAllowedResult(undefined)).toBe(false));
  ...
  it('empty string → blocked', () => expect(isEmailAllowedResult('' as unknown as boolean)).toBe(false));
});
