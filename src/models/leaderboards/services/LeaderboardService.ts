...
export class LeaderboardService {
  ...
  async getUserRankings(userId: string): Promise<UserRankingSummary | null> {
    const [userResult, rankResult] = await Promise.all([
      this.supabaseClient
        .from("users")
        .select(
          "id, created_at, updated_at, username, first_name, last_name, profile_icon_url, school_id, house_id, year_group, is_public, role, is_active, is_admin, total_points, total_kilometers, total_minutes, monthly_goal_minutes, current_streak, longest_streak, last_activity_date, school_rank, house_rank, year_group_rank, overall_rank, school:schools(id, name, is_internal), house:houses(id, name, color)",
        )
        .eq("id", userId)
        .single(),
      this.supabaseClient.rpc("get_user_rankings", { p_user_id: userId }).single(),
    ]);
  ...
  async getSchoolLeaderboard(userSchoolId?: string): Promise<SchoolLeaderboardEntry[]> {
    ...
    let query = this.supabaseClient.from("schools").select("*").eq("is_active", true);
    ...
    const { data: schools, error } = await query.order("term_points", { ascending: false });
  ...
  async getHouseLeaderboard(schoolId: string): Promise<HouseLeaderboardEntry[]> {
    const { data: houses, error } = await this.supabaseClient
      ...
      .order("term_points", { ascending: false });
    ...
    const leaderboard: HouseLeaderboardEntry[] = houses.map((house, index) => {
      ...
      const termScore = house.term_points || house.total_points || 0;
      const averagePointsPerMember = memberCount > 0 ? termScore / memberCount : 0;
  ...
  async getOverallLeaderboard(filters?: LeaderboardFilters): Promise<IndividualLeaderboardEntry[]> {
    let query = this.supabaseClient
      .from("users")
      .select(
        "id, created_at, updated_at, username, first_name, last_name, profile_icon_url, is_public, school_id, house_id, year_group, role, is_active, is_admin, total_points, total_kilometers, total_minutes, monthly_goal_minutes, current_streak, longest_streak, last_activity_date, school_rank, house_rank, year_group_rank, overall_rank, school:schools(id, name), house:houses(id, name, color)",
      )
      .eq("is_deleted", false)
