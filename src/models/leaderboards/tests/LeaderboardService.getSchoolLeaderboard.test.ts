...
const SCHOOL_C = makeSchool({ id: 'c', name: 'School C (Internal)', is_internal: true, total_points: 2000, total_students: 20 });

function setupLeaderboardMock(supabase: ReturnType<typeof makeSupabaseMock>, {
  userSchool,
  schools,
}: {
...
describe('LeaderboardService.getSchoolLeaderboard — internal school filtering', () => {
    ...
    supabase.from = vi.fn().mockImplementation((table: string) => {
      ...
      if (table === 'schools') {
        ...
        chain.single = vi.fn().mockImplementation(() => {
          callCount++;
          if (callCount === 1) return Promise.resolve({ data: { is_internal: false }, error: null });
  ...
  it('no userSchoolId → all active schools returned (admin/unauthenticated)', async () => {
    supabase.from = vi.fn().mockImplementation((table: string) => {
      ...
      chain.select = vi.fn().mockReturnThis();
      chain.eq = vi.fn().mockReturnThis();
      chain.order = vi.fn().mockResolvedValue({ data: [SCHOOL_A, SCHOOL_B, SCHOOL_C], error: null });
      return chain;
    });
  ...
  it('pro-rata ranking: school with higher pts/student ranks above school with more total pts', async () => {
    ...
    const schoolX = makeSchool({ id: 'x', name: 'Small School', total_points: 200, total_students: 10 });
    const schoolY = makeSchool({ id: 'y', name: 'Big School', total_points: 1000, total_students: 100 });
  ...
  it('school with 0 students gets pro_rata_score of 0 (no division by zero)', async () => {
    const zeroSchool = makeSchool({ id: 'z', name: 'Empty School', total_points: 500, total_students: 0 });
