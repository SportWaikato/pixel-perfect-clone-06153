  ...
  async create(activityData: Partial<ActivityInterface>): Promise<ActivityInterface> {
    ...
    if (activityData.event_id) {
      ...
      if (event?.challenge_points) {
        const capped = event?.max_points
          ? Math.min(event.challenge_points, event.max_points)
        ...
        if (event.max_points) {
          pointsCalculation = calculatePointsWithMultiplier(
            Math.min(basePoints, event.max_points),
            eventMultiplier,
          );
        } else {
