...
import { MAX_ACTIVITY_DURATION_MINUTES, MAX_ACTIVITY_DAYS_AGO } from '../constants/activityValidationConstants';
...
describe('ActivityService.update — ownership and validation', () => {
  ...
  it('throws "Activity not found" when getById returns null', async () => {
    ...
    await expect(service.update('act-1', 'user-1', { duration_minutes: 30 })).rejects.toThrow('Activity not found');
  });
  ...
  it('throws "You can only edit your own activities" when user_id does not match', async () => {
    ...
    await expect(service.update('act-1', 'user-2', { duration_minutes: 30 })).rejects.toThrow('You can only edit your own activities');
  });
  ...
  it('does NOT throw when duration_minutes is omitted (valid partial update)', async () => {
    ...
    supabase._chain.single
      ...
      .mockResolvedValueOnce({ data: { ...EXISTING_ACTIVITY, activity_type: 'running' }, error: null }); // update
    supabase.rpc.mockResolvedValue({ data: null, error: null });

    await expect(service.update('act-1', 'user-1', { activity_type: 'running' })).resolves.toBeDefined();
  });

  it('throws when duration_minutes is explicitly 0', async () => {
    supabase._chain.single.mockResolvedValue({ data: EXISTING_ACTIVITY, error: null });
    await expect(
      service.update('act-1', 'user-1', { duration_minutes: 0 })
  ...
  it('does NOT throw when created_at is omitted (date validation skipped)', async () => {
    ...
    await expect(service.update('act-1', 'user-1', { duration_minutes: 30 })).resolves.toBeDefined();
  });
});
...
describe('ActivityService.update — points recalculation', () => {
  ...
  it('uses calculatePointsWithMultiplier (base only) when duration changes and no event', async () => {
    ...
    const expectedActivity = { ...EXISTING_ACTIVITY, duration_minutes: 30, base_points: 30, final_points: 30, house_points_awarded: 30 };
  ...
  it('uses calculateFixedChallengePoints when event has challenge_points', async () => {
    ...
    const expectedActivity = { ...EXISTING_ACTIVITY, base_points: 0, final_points: 100, house_points_awarded: 100, challenge_points_multiplier: 100 };
    setupUpdate(eventWithChallenge, expectedActivity);

    const result = await service.update('act-1', 'user-1', { event_id: 'event-1', duration_minutes: 60 });
  ...
  it('uses calculatePointsWithMultiplier with event multiplier when event has no challenge_points', async () => {
    ...
    const expectedActivity = { ...EXISTING_ACTIVITY, base_points: 60, final_points: 120, house_points_awarded: 60 };
    setupUpdate(eventWithMultiplier, expectedActivity);

    const result = await service.update('act-1', 'user-1', { event_id: 'event-2', duration_minutes: 60 });
