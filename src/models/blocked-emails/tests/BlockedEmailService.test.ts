...
describe("BlockedEmailService", () => {
  ...
  describe("add", () => {
    ...
    it("throws on database error", async () => {
      ...
      await expect(service.add(SCHOOL_ID, "bad@school.com", USER_ID)).rejects.toThrow(
        "insert failed",
      );
    });
  });
  ...
  describe("bulkAdd", () => {
    ...
    it("deduplicates emails within the input", async () => {
      ...
      await service.bulkAdd(
        SCHOOL_ID,
        ["bad@school.com", "BAD@school.com", "bad@school.com"],
        USER_ID,
      );
    ...
    it("correctly separates added from skipped (already-blocked) emails", async () => {
      ...
      const result = await service.bulkAdd(
        SCHOOL_ID,
        ["newbad@school.com", "alreadyblocked@school.com"],
        USER_ID,
      );
    ...
    it("throws on database error", async () => {
      ...
      await expect(service.bulkAdd(SCHOOL_ID, ["bad@school.com"], USER_ID)).rejects.toThrow(
        "insert failed",
      );
    });
  });
