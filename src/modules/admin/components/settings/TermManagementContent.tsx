...
import { Plus, Edit2, Trash2, Calendar, Monitor, RefreshCw, CheckCircle2 } from "lucide-react";
...
const TermManagementContent = ({
  terms: initialTerms,
  schoolId,
  currentUser,
}: TermManagementContentProps) => {
  ...
  const handleResetTermPoints = async () => {
    const supabase = createSupabaseClient();
    const schoolService = new SchoolService(supabase);
    try {
      await schoolService.resetTermPoints(schoolId);
      toast.success("House and school term points reset to zero. Lifetime totals preserved.");
    } catch (error) {
      notifyAboutError(error);
    }
  };

  const handleSetCurrentTerm = async (term: SchoolTermInterface) => {
    const supabase = createSupabaseClient();
    const service = new SchoolTermService(supabase);
    try {
      await service.setCurrentTerm(schoolId, term.id);
      setTerms((prev) =>
        prev.map((t) => ({
          ...t,
          is_current: t.id === term.id,
        })),
      );
      toast.success(`Term ${term.term_number} ${term.year} set as current`);
    } catch (error) {
      notifyAboutError(error);
    }
  };
        ...
        <div className="flex-1 px-5 py-4">
          {term ? (
            <div className="space-y-1">
              ...
              {currentWeek && (
                <p className="text-xs font-medium text-[#00ACEF]">
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      This will remove Term {termNumber} {selectedYear} from this school. Assembly
                      period data will no longer reference this term.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
      ...
      >
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editingTerm
                ? `Edit Term ${editingTerm.term_number} ${selectedYear}`
                : `Add Term ${initialTermNumber} ${selectedYear}`}
            </DialogTitle>
          </DialogHeader>
          <Formik
            enableReinitialize
