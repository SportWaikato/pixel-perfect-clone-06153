      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Enter the email address for the new super admin. Share the generated link with them —
            they&apos;ll use it to create their account.
          </p>
        </CardHeader>
          ...
          {generatedUrl && (
            <div className="rounded-lg border bg-gray-50 p-4 space-y-2">
              ...
              <div className="flex items-center gap-2">
                <Input readOnly value={generatedUrl} className="font-mono text-xs bg-white" />
                <Button
                  size="sm"
                  variant="outline"
        ...
        <CardContent className="space-y-3">
          ...
          {(() => {
            ...
            const page = filtered.slice(
              invitePage * INVITE_PAGE_SIZE,
              (invitePage + 1) * INVITE_PAGE_SIZE,
            );
              ...
              <>
                ...
                {filtered.length > INVITE_PAGE_SIZE && (
                  <div className="flex items-center justify-between mt-4 pt-3 border-t text-sm text-gray-500">
                    <span>
                      {invitePage * INVITE_PAGE_SIZE + 1}–
                      {Math.min((invitePage + 1) * INVITE_PAGE_SIZE, filtered.length)} of{" "}
                      {filtered.length}
                    </span>
