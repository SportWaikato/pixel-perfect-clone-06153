...
const EditEventDialog = ({ open, onOpenChange, event, onEventUpdated, user }: EditEventDialogProps) => {
  ...
  const [eventImageState, setEventImageState] = useState<EventImageState | null>(
    event?.event_image_url ? { url: event.event_image_url, path: event.event_image_storage_path || '' } : null
  );
  ...
  const audienceInfo = useMemo((): { audience: EventAudienceOption; selectedSchoolIds: string[] } => {
    if (!event) {
      return {
  ...
  const handleSubmit = async (
    ...
    try {
      if (isSuperAdmin && values.audience === 'sport_waikato' && sportWaikatoSchoolIds.length === 0) {
                      ...
                      <div className="space-y-6 rounded-xl border border-gray-200 bg-gray-50 p-5">
                        ...
                        <div className="space-y-2">
                          ...
                          <BadgeImagePicker
                            ...
                            onSelect={handleBadgeImageSelect}
                          />
                        </div>
