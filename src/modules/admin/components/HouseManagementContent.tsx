...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger, DialogDescription } from '@/modules/application/components/DesignSystem/ui/dialog';
...
const predefinedColors = [
  ...
  '#FFB74D', '#9575CD', '#42A5F5', '#66BB6A',
  '#FF8A65', '#BA68C8', '#26A69A', '#8BC34A',
  '#FFA726', '#7986CB', '#EF5350', '#29B6F6'
];

const HouseManagementContent = ({ user, backHref, schoolId, schoolName, schools = [] }: HouseManagementContentProps) => {
  ...
  const { data: houses, filteredData: filteredHouses, loading, searchTerm, setSearchTerm, refresh: fetchHouses, setData: setHouses } = useAdminData({
  ...
  const handleUpdateHouse = async (values: HouseFormData, { setSubmitting }: any) => {
    ...
    try {
      ...
      const nameExists = await houseService.checkNameExists(values.name.trim(), effectiveSchoolId!, editingHouse.id);
      if (nameExists) {
        toast.error('A house with this name already exists in your school');
        setSubmitting(false);
        return;
  ...
  const HouseForm = ({ initialValues, onSubmit, submitText }: {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600">
              {effectiveSchoolName ? `Manage house teams for ${effectiveSchoolName}` : 'Select a school to manage houses'}
            </p>
          </div>
        </div>
        ...
        {effectiveSchoolId && (
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Add a new house team to your school. Each house should have a unique name and color.
              </DialogDescription>
            </DialogHeader>
            <HouseForm 
        ...
        <CardContent>
          {filteredHouses.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              {searchTerm ? 'No houses found matching your search.' : 'No houses created yet. Create your first house to get started!'}
            </div>
          ) : (
            <div className="space-y-2">
              {filteredHouses.map((house) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Dialog open={editingHouse?.id === house.id} onOpenChange={(open) => {
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            ...
                            Students currently assigned to this house will need to be reassigned to another house.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
