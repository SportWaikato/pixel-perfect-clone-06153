...
const validationSchema = object().shape({
  ...
  registration_method: string()
    .oneOf(['domain_blocklist', 'allowlist'])
    .required(),
  email_domain: string()
    .nullable()
    .when('registration_method', {
      ...
      then: (s) =>
        s
          ...
          .test('valid-domains', 'Enter valid domains separated by commas (e.g. school.co.nz, other.edu)', (value) => {
            if (!value) return false;
            const domainRegex = /^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/;
            return value.split(',').map(d => d.trim()).filter(Boolean).every(d => domainRegex.test(d));
          })
          .max(255, 'Domains must be less than 255 characters'),
      otherwise: (s) =>
        s
          .test('valid-domains', 'Enter valid domains separated by commas (e.g. school.co.nz)', (value) => {
...
const registrationOptions = [
  {
    ...
    description: 'Students with a matching school email domain can register. Block individual addresses if needed.',
    icon: Globe,
  },
  {
            ...
            <div key={option.value}>
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  >
                    {isSelected && (
                      <div className="h-2 w-2 rounded-full bg-[#0B4B39]" />
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <Icon size={14} className={isSelected ? 'text-[#0B4B39]' : 'text-gray-400'} />
                      <span className={cn('text-sm font-medium', isSelected ? 'text-[#0B4B39]' : 'text-gray-700')}>
                        {option.label}
                      </span>
                    </div>
                    ...
                    {({ field, meta }: any) => (
                      <div className="space-y-1">
                        ...
                        <Input
                          ...
                          disabled={isSubmitting}
                          className={cn('h-8 text-sm', meta.touched && meta.error ? 'border-red-500' : '')}
                        />
                        <p className="text-xs text-gray-400">Separate multiple domains with commas</p>
  ...
  const handleSubmit = async (values: FormValues, { setSubmitting }: any) => {
    try {
      const schoolData = {
        ...
        email_domain: values.registration_method === 'domain_blocklist'
          ? values.email_domain?.split(',').map(d => d.trim().toLowerCase()).filter(Boolean).join(', ') || null
          : null,
        is_active: isActive,
