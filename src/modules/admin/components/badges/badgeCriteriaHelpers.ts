...
export const CRITERIA_TYPE_OPTIONS = [
  ...
  },
  {
    ...
    description: 'Automatically awarded when a user completes their first challenge-linked activity.',
  },
  {
...
export const badgeFormValidationSchema = object()
  .shape({
  ...
  activityType: string()
    ...
    .default(null)
    .when('criteriaType', {
      is: (criteriaType: CriteriaType) => ['specific_activity', 'walk_and_talk'].includes(criteriaType),
      then: schema => schema.required('Select an activity type'),
      otherwise: schema => schema.optional(),
    }),
  durationMinutes: badgeNumberStringSchema().when('criteriaType', {
    is: (criteriaType: CriteriaType) => ['specific_activity', 'social_activity', 'time_in_nature', 'walk_and_talk'].includes(criteriaType),
  ...
  participationType: string()
    .default('')
    .when('criteriaType', {
      is: (criteriaType: CriteriaType) => ['social_activity', 'walk_and_talk'].includes(criteriaType),
      then: schema => schema.oneOf(PARTICIPATION_OPTIONS.map(option => option.value)).required('Select participation type'),
      otherwise: schema => schema.oneOf(['', ...PARTICIPATION_OPTIONS.map(option => option.value)]),
    }),
...
export const getInitialBadgeFormValues = (badge: AchievementInterface | null): BadgeFormValues => {
  const rawCriteria = badge?.criteria && typeof badge.criteria === 'object' ? badge.criteria : defaultCriteria;
  ...
  const baseValues: BadgeFormValues = {
    ...
    dateRangeStart: rawCriteria?.date_range?.start || '',
    dateRangeEnd: rawCriteria?.date_range?.end || '',
  };
  ...
  if (!baseValues.participationType && ['social_activity', 'walk_and_talk'].includes(criteriaType)) {
