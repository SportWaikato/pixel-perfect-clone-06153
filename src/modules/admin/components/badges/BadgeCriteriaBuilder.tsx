...
export const BadgeCriteriaBuilder = ({
  ...
  const scopeValue: string | undefined = namePrefix
    ? ((values as unknown as Record<string, unknown>)[namePrefix] as { scope?: string } | undefined)
        ?.scope
    : values.scope;
    ...
    switch (values.criteriaType) {
      case "specific_activity":
        return (
          <>
              ...
              <FormikSelectField
                ...
                onValueChange={(value) => {
                  if (value !== "something_else") {
                    setFieldValue(fieldName("customActivityName"), "");
                  }
                }}
              >
            ...
            {values.activityType === "something_else" && (
              <FormikInputField
                name={fieldName("customActivityName")}
                label="Custom Activity Name"
                placeholder="e.g. Star Jumps"
              />
            )}
          </>
        );
      ...
      case "leaderboard_entry":
        return (
          <div className="grid gap-4 md:grid-cols-2">
            <FormikInputField
              name={fieldName("leaderboardRank")}
              type="number"
              label="Top Positions"
              placeholder="10"
            />
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <FormikSwitchField
          ...
          description={
            scopeValue === "house"
              ? "The house challenge will be evaluated within the selected dates."
              : "Learners must complete the badge between the selected dates."
          }
        />
