...
import { getNZDate, getActivityDisplayName, getUserInitials, computeGroupRejectableIds } from './activityUtils';
import { ActivityInterface } from '@/models/activities/interfaces/ActivityInterface';

// Minimal activity factory for these tests
function makeActivity(overrides: Partial<ActivityInterface> & { id: string; user_id: string; created_at: string }): ActivityInterface {
  return {
...
describe('getActivityDisplayName', () => {
  it('returns custom_activity_name when type is something_else', () => {
    const a = makeActivity({ id: '1', user_id: 'u1', created_at: JAN15_UTC_AFTERNOON, activity_type: 'something_else', custom_activity_name: 'Juggling' });
    expect(getActivityDisplayName(a)).toBe('Juggling');
  });

  it('returns ACTIVITY_TYPES label for known type', () => {
    const a = makeActivity({ id: '1', user_id: 'u1', created_at: JAN15_UTC_AFTERNOON, activity_type: 'run_jog' });
    expect(getActivityDisplayName(a)).toBe('Run / Jog');
  });

  it('falls back to replace("_", " ") for unknown type not in ACTIVITY_TYPES', () => {
    const a = makeActivity({ id: '1', user_id: 'u1', created_at: JAN15_UTC_AFTERNOON, activity_type: 'made_up_type' });
...
describe('getUserInitials', () => {
  ...
  it('returns uppercased initials from first_name and last_name', () => {
    const a = makeActivity({ id: '1', user_id: 'u1', created_at: JAN15_UTC_AFTERNOON, user: { first_name: 'Jane', last_name: 'Smith' } as any });
    expect(getUserInitials(a)).toBe('JS');
  });

  it('handles missing first_name gracefully', () => {
    const a = makeActivity({ id: '1', user_id: 'u1', created_at: JAN15_UTC_AFTERNOON, user: { first_name: '', last_name: 'Smith' } as any });
    expect(getUserInitials(a)).toBe('S');
  });

  it('handles missing last_name gracefully', () => {
    const a = makeActivity({ id: '1', user_id: 'u1', created_at: JAN15_UTC_AFTERNOON, user: { first_name: 'Jane', last_name: '' } as any });
...
describe('computeGroupRejectableIds', () => {
  ...
  it('rejected activity on flagged day: in map, maps to the non-rejected siblings (not itself)', () => {
    const activities = [
      makeActivity({ id: 'a1', user_id: 'u1', created_at: DAY, is_flagged: true }),
      makeActivity({ id: 'a2', user_id: 'u1', created_at: DAY, is_flagged: true, is_rejected: true }),
      makeActivity({ id: 'a3', user_id: 'u1', created_at: DAY, is_flagged: true }),
    ];
