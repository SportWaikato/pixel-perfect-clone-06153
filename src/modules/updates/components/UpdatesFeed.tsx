...
const UpdatesFeed = ({ user, initialUpdates = [], embedded = false }: UpdatesFeedProps) => {
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            {!update.is_read && <span className="w-2 h-2 rounded-full bg-blue-500 shrink-0" />}
            <p
          ...
          {update.body ? (
            <p
              className={`text-sm text-gray-800 mt-0.5${isExpanded ? " whitespace-pre-wrap" : ""}`}
            >
    ...
    <div className="bg-white rounded-xl p-6 space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-600 mt-1">
          Use this form to send us a message, suggestions or feedback. Your name, school and class
          year will be automatically captured.
        </p>
      </div>
      <Formik
  ...
  const updatesList = loading ? (
    ...
    <div className="space-y-3">{updates.map(renderUpdateItem)}</div>
  );
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="bg-white rounded-xl p-6 space-y-4">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Use this form to send us a message, suggestions or feedback. Your name, school and
              class year will be automatically captured.
            </p>
          </div>
          <Formik
            initialValues={{ message: "" }}
