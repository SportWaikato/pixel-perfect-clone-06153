...
const KoreroVotingForm = ({ user }: KoreroVotingFormProps) => {
  const [existingVote, setExistingVote] = useState<KoreroVoteInterface | null>(null);
  const [loading, setLoading] = useState(true);
  const [hasVoted, setHasVoted] = useState(false);

  const koreroVoteService = new KoreroVoteService(createSupabaseClient());

  useEffect(() => {
    const fetchExistingVote = async () => {
      try {
        const vote = await koreroVoteService.getUserVote(user.id);
  ...
  const handleSubmit = async (values: any, { setSubmitting }: any) => {
    try {
      ...
      const updatedVote = await koreroVoteService.getUserVote(user.id);
