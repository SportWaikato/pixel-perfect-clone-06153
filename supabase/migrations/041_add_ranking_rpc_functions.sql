-- get_user_rankings: returns all 4 ranking dimensions for a single user in one query
-- Replaces 12 individual COUNT queries previously fired from LeaderboardService
...
-- recalculate_all_rankings: updates rank columns for every user in a single UPDATE
-- Replaces the JS batching loop that previously fired ~13 queries per user
...
-- Recalculate all totals so the leaderboard reflects current activity data
UPDATE users u
SET
